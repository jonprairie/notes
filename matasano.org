...
#+BEGIN_SRC python :tangle matasano.py
  ...
  def fixed_xor(b1, b2):
      """takes two lists of bytes b1 and b2 and produces a list of bytes
   consisting of the bitwise xor's of corresponding bytes in b1 and b2"""

      assert(len(b1) == len(b2))

      return [p[0] ^ p[1] for p in zip(b1, b2)]
#+END_SRC
...
#+BEGIN_SRC python :tangle matasano.py
  def get_ch7_file():
      with open('matasano/ch7.txt', 'r') as f:
          return f.read().splitlines()

  import Crypto.Cipher.AES as AES
  def decrypt_bytes_aes_ecb(byte_list, key):
      decrypt = AES.new(key, AES.MODE_ECB).decrypt
      return list(decrypt(bytes(byte_list)))
#+END_SRC
...
#+BEGIN_SRC python :tangle matasano.py
  def get_ch8_file():
      with open('matasano/ch8.txt', 'r') as f:
          return f.read().splitlines()

  import math
  def chunk(l, chunk_size):
      """chunk an iterable into a list of iterables, each of size chunk_size"""
      return_size = math.ceil(len(l) / chunk_size)
      return [
          l[k*chunk_size:(k+1)*chunk_size]
          for k in range(return_size)
      ]

  def count_discrete_repeats(byte_list, size=16):
      split_bytes = chunk(byte_list, size)
      repeats = map(lambda p: 1 if p[1] in split_bytes[p[0]+1:] else 0, enumerate(split_bytes))
      return sum(repeats)
#+END_SRC
...
**** solution:
#+BEGIN_SRC python :tangle matasano.py
  def pad(byte_list, block_size=16):
      """pad a byte_list to be a multiple of block_size by appending 
  bytes of value k*block_size - len(byte_list), where k is the smallest 
  integer such that k*block_size > len(byte_list)"""

      block_size_diff = -len(byte_list) % block_size
      
      return byte_list + [block_size_diff] * block_size_diff

  def pad_msg(s, block_size=16):
      byte_list = list(s.encode('ascii'))
      padded_bytes = pad(byte_list, block_size)
      return bytes(padded_bytes).decode('ascii')
#+END_SRC
...
#+BEGIN_SRC python :tangle matasano.py
  def get_ch10_data():
      with open('matasano/ch10.txt', 'r') as f:
          return f.read().splitlines()

  def rotate_left(l, n):
      """rotate l, an iterable, n elements to the left, appending the first n elements to the end"""
      mod_n = n % len(l)
      return l[mod_n:] + l[:mod_n]

  import Crypto.Cipher.AES as AES
  def encrypt_bytes_aes_ecb(byte_list, key):
      assert(len(key) in [16, 24, 32])
      assert(len(byte_list) % 16 == 0)

      encrypt = AES.new(key, AES.MODE_ECB).encrypt
      return list(encrypt(bytes(byte_list)))

  ENCRYPT = 1
  DECRYPT = 2
  def cbc_128(direction, func, byte_list, key, iv):
      assert(len(key) in [16, 24, 32])
      assert(len(byte_list) % 16 == 0)
      assert(len(iv) == 16)
      ...
      if byte_list:
          if direction == ENCRYPT:
              prepared_bytes = fixed_xor(byte_list[:16], iv)
              processed_bytes = func(prepared_bytes, key)
              next_iv = processed_bytes
          else:
              prepared_bytes = func(byte_list[:16], key)
              processed_bytes = fixed_xor(prepared_bytes, iv)
              next_iv = byte_list[:16]
          return processed_bytes + cbc_128(direction, func, byte_list[16:], key, next_iv)
      else:
          return []

  def encrypt_bytes_aes_cbc(byte_list, key, iv):
      return cbc_128(ENCRYPT, encrypt_bytes_aes_ecb, byte_list, key, iv)

  def decrypt_bytes_aes_cbc(byte_list, key, iv):
      return cbc_128(DECRYPT, decrypt_bytes_aes_ecb, byte_list, key, iv)
#+END_SRC

*** Challenge 11: Implement an ECB/CBC Detection Oracle
...
#+BEGIN_SRC python :tangle matasano.py
  ...
  def gen_random_bytes(length):
      return [random.randint(0,127) for n in range(length)]

  def random_encrypt(msg):
      prepend_bytes = gen_random_bytes(random.randint(5,10))
      append_bytes = gen_random_bytes(random.randint(5,10))
      padded_bytes = prepend_bytes + list(msg.encode('ascii')) + append_bytes

      sized_bytes = pad(padded_bytes)
      
      rand_key = bytes(gen_random_bytes(16)).decode('utf-8')

      if random.randint(0,1):
          rand_iv = gen_random_bytes(16)
          return encrypt_bytes_aes_cbc(sized_bytes, rand_key, rand_iv)
      else:
          return encrypt_bytes_aes_ecb(sized_bytes, rand_key)

  ECB = 1
  CBC = 2
  def cbc_or_ecb_oracle(func):
      msg = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
      if count_discrete_repeats(func(msg)):
          return ECB
      else:
          return CBC
#+END_SRC
